        ...
        <section className="w-full p-3 sm:p-4 lg:p-6">
          ...
          <header className="rounded-xl border border-slate-200 bg-white px-3 py-3 sm:px-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex w-full flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white px-3 py-2 text-sm text-slate-700 shadow-sm sm:w-auto sm:rounded-2xl">
                  <input
                    type="date"
                    value={startDate}
                    onChange={(event) => setStartDate(event.target.value)}
                    className="h-9 w-full rounded-lg border border-slate-300 bg-white px-2 text-xs text-slate-700 outline-none transition focus:border-slate-400 sm:w-36"
                  ...
                  <button
                    ...
                    className="h-9 rounded-lg bg-slate-900 px-3 text-xs font-semibold text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-400"
                  >
                    Apply
                  </button>
                  <button
                    onClick={clearDateFilter}
                    className="h-9 rounded-lg border border-slate-300 bg-white px-3 text-xs font-medium text-slate-600 transition hover:bg-slate-100"
                  >
                    Clear
                  </button>
                </div>
                <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm text-slate-500 shadow-sm">
                  <MagnifyingGlassIcon className="h-4 w-4" />
                  Search
                </div>
              </div>
            </div>
            <p className="mt-2 text-xs text-slate-500">Current data window: {overviewData.dateRangeLabel}</p>
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-12">
            <article className="rounded-2xl bg-[#ff3b3b] p-5 text-white xl:col-span-4">
              ...
              <button className="mt-8 text-sm font-medium text-white/90">See Details →</button>
            </article>
            ...
            <article className="rounded-2xl border border-slate-200 bg-white p-5 xl:col-span-4">
              <p className="text-2xl font-semibold text-slate-800">Misinformation Ratio</p>
              <MisinformationRatioChart data={overviewData.ratioData} />
              <div className="mt-2 flex gap-4 text-xs text-slate-600">
                <p><span className="inline-block h-2 w-2 rounded-full bg-[#9acb3f]" /> Verified</p>
                <p><span className="inline-block h-2 w-2 rounded-full bg-[#f59e0b]" /> Misinformation</p>
              </div>
              <p className="mt-2 text-xs text-slate-500">Sentiment mapping will be enabled once labeled data is available.</p>
            </article>
            ...
            <article className="rounded-2xl border border-slate-200 bg-white p-5 xl:col-span-8">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-4">
                {overviewData.heatMapData.map((item) => (
              ...
              <p className="mt-3 text-xs text-slate-500">Heat scale by tweet volume from UserLocation: Low, Medium, High</p>
            </article>
            ...
            <article className="rounded-2xl border border-slate-200 bg-white p-5 xl:col-span-2">
              ...
              <p className="mt-1 text-sm text-slate-600">Pending sentiment labels</p>
            </article>

            <article className="rounded-2xl bg-[#2f7f76] p-5 text-white xl:col-span-2">
              <p className="text-lg font-semibold">Last Data Scrape</p>
